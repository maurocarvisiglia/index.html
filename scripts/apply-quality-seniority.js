...
// Gruppo Manager: QP (Qualified Person) e Supervisor
const groupManager = [/\bqp\b/i, /qualified person/i, /farmacista/i, /supervisor/i, /supervisore/i, /\bhead\b/i];
// Tutto il resto (tecnici, QC, ingegneri, consulenti HSE/RSPP) -> specialist,
// con RAL come variabile secondaria per eventuale upgrade a senior_specialist

function classifyBase(title) {
  if (groupManager.some(p => p.test(title))) return 'manager';
  return 'specialist';
}

async function run() {
  console.log('🔧 CLASSIFICAZIONE seniority_v2 — quality\n');
  console.log('═'.repeat(80));

  const { data: jobs } = await supabase
    .from('job_listings')
    .select('id, job_title, ral_min, ral_max')
    .eq('functional_area_v2', 'quality')
  ...
  const dist = new Map();
  for (const j of jobs) {
    let value = classifyBase(j.job_title);

    if (value === 'specialist' && j.ral_min && j.ral_max) {
