...
// Gruppo entry_level: ruoli manuali di magazzino/trasporto
const groupEntry = [/magazzin/i, /warehouse/i, /\bautista\b/i, /\bcorriere\b/i];

async function run() {
  console.log('🔧 CLASSIFICAZIONE seniority_v2 — supply_chain\n');
  console.log('═'.repeat(80));

  const { data: jobs } = await supabase
    .from('job_listings')
    .select('id, job_title, ral_min, ral_max')
    .eq('functional_area_v2', 'supply_chain')
  ...
  for (const j of jobs) {
    let value;
    if (groupEntry.some(p => p.test(j.job_title))) {
      value = 'entry_level';
    } else {
      value = 'specialist';
