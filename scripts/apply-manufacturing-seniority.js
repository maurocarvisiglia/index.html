import { createClient } from '@supabase/supabase-js';
import dotenv from 'dotenv';
dotenv.config();
const supabase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);

// Gruppo B: ingegneri/progettisti/scienziati di processo -> specialist
const groupB = [/ingegnere/i, /\bengineer\b/i, /progettista/i, /designer/i, /process scientist/i, /msat scientist/i, /\bscientist\b/i];
// Gruppo C: supervisori/capiturno/coordinatori -> lead
const groupC = [/supervisor/i, /supervisore/i, /capoturno/i, /discipline leader/i, /\blead\b/i, /planner/i, /coordinator/i];
// Gruppo A (default): operai/tecnici/manutentori/addetti -> entry_level

function classify(title) {
  if (groupC.some(p => p.test(title))) return 'lead';
  if (groupB.some(p => p.test(title))) return 'specialist';
  return 'entry_level';
}

async function run() {
  console.log('🔧 CLASSIFICAZIONE seniority_v2 — manufacturing\n');
  console.log('═'.repeat(80));

  const { data: jobs } = await supabase
    .from('job_listings')
    .select('id, job_title')
    .eq('functional_area_v2', 'manufacturing')
    .is('seniority_v2', null);

  console.log(`Trovati: ${jobs.length}`);

  const dist = new Map();
  for (const j of jobs) {
    const value = classify(j.job_title);
    await supabase.from('job_listings').update({ seniority_v2: value }).eq('id', j.id);
    dist.set(value, (dist.get(value) || 0) + 1);
  }

  console.log('\nDistribuzione applicata:');
  Array.from(dist.entries()).sort((a,b)=>b[1]-a[1]).forEach(([v,c]) => console.log(`   ${v}: ${c}`));

  console.log('\n📊 STATO FINALE seniority_v2 (globale)...');
  const { count: totalJobs } = await supabase.from('job_listings').select('*', { count: 'exact', head: true });
  const { count: withSen } = await supabase.from('job_listings').select('*', { count: 'exact', head: true }).not('seniority_v2', 'is', null);
  console.log(`   Con seniority_v2: ${withSen}/${totalJobs} (${((withSen/totalJobs)*100).toFixed(1)}%)`);
  console.log('\n' + '═'.repeat(80));
}
run();
