...
if (!fa) { console.log('Uso: node apply-generic-ral-tier.js <functional_area_v2>'); process.exit(1); }

async function run() {
  console.log(`🔧 CLASSIFICAZIONE seniority_v2 — ${fa}\n`);
  ...
  for (const j of jobs) {
    ...
    if (j.ral_min && j.ral_max) {
      ...
      if (avg < 28000) value = 'entry_level';
      else if (avg >= 45000) value = 'senior_specialist';
    }
